...
export default function App() {
  ...
  // Performance Optimization (Bolt): Derive distance & bearing dynamically via useMemo
  // to avoid mutating `targets` state on high-frequency GPS ticks, preventing unnecessary
  // localStorage serialization and Leaflet marker re-renders.
  ...
  // Base targets filtered for MapRadarCanvas (stable during GPS movement)
  ...
  // Filtered candidate list with distance/bearing for sidebar & navigation
  ...
  // Selected target with live GPS distance/bearing metrics
  ...
  return (
    <div className="relative w-screen h-screen overflow-hidden bg-[#0d1117] text-stone-100 flex flex-col font-sans-ui">
      ...
      <main className="relative flex-1 w-full h-full overflow-hidden">
        ...
        {/* ------------------------------------------------------------------- */}
        {currentSelectedTarget && !isPopupOpen && (
          <div
          ...
          >
            <div className="flex items-start justify-between gap-2">
              <div>
                <span className="text-[10px] font-mono-tech uppercase text-emerald-400 font-semibold">
                  {currentSelectedTarget.category.replace(/_/g, ' ')} • {currentSelectedTarget.township} Twp
                </span>
                <h3 className="text-base font-bold font-display text-emerald-100">
                  {currentSelectedTarget.name}
                </h3>
              </div>
            ...
            <div className="flex items-center justify-between text-xs font-mono-tech text-stone-300 my-2.5 p-2 rounded-lg bg-black/40 border border-emerald-900/40">
              <span>EST. SETTLED: {currentSelectedTarget.yearSettled}</span>
              {currentSelectedTarget.distanceMeters ? (
                <span className="text-cyan-300 font-bold">
                  RANGE: {Math.round(currentSelectedTarget.distanceMeters)}m
                </span>
              ) : (
            ...
            <div className="grid grid-cols-3 gap-2 pt-1">
              <button
                ...
                onClick={() => {
                  ...
                  handleStatusChange(currentSelectedTarget.id, 'confirmed');
                }}
                className={`py-2 px-1 rounded-xl text-xs font-mono-tech font-bold border flex flex-col items-center justify-center gap-1 transition active:scale-95 ${
                  currentSelectedTarget.verificationStatus === 'confirmed'
              ...
              <button
                ...
                onClick={() => {
                  ...
                  triggerHaptic(40);
                  handleStatusChange(currentSelectedTarget.id, 'walkover');
                }}
                className={`py-2 px-1 rounded-xl text-xs font-mono-tech font-bold border flex flex-col items-center justify-center gap-1 transition active:scale-95 ${
                  currentSelectedTarget.verificationStatus === 'walkover'
      ...
      <MarkerDetailPopup
        ...
        userDistanceMeters={currentSelectedTarget?.distanceMeters}
        userBearingDeg={currentSelectedTarget?.bearingDeg}
        isGpsActive={isGpsActive}
        ...
        isLockedTarget={lockedTarget?.id === currentSelectedTarget?.id}
      />
